package com.safeandfast.dto.response;

public class ResponseMessage {

    public final static String CONTACTMESSAGE_SAVE_RESPONSE_MESSAGE= "ContactMessage Successfully Created";

    public final static String CONTACTMESSAGE_DELETE_RESPONSE_MESSAGE= "ContactMessage Successfully Created";

    public final static String CONTACTMESSAGE_UPDATE_RESPONSE_MESSAGE= "ContactMessage Successfully Updated";

    public final static String REGISTER_RESPONSE_MESSAGE= "Registration successfully done";

    public final static String PASSWORD_CHANGE_RESPONSE_MESSAGE= "Password Successfull Change";

    public final static String USER_UPDATE_RESPONSE_MESSAGE= "User Successfully Updated";

    public final static String USER_DELETE_RESPONSE_MESSAGE= "User Successfully Deleted";

    public final static String IMAGE_SAVED_RESPONSE_MESSAGE= "Image Successfully Uploaded";

    public final static String IMAGE_DELETE_RESPONSE_MESSAGE= "Image Successfully Deleted";

    public final static String CAR_SAVED_RESPONSE_MESSAGE= "Car Successfully Uploaded";

    public final static String CAR_UPDATE_RESPONSE_MESSAGE= "Car Successfully Updated";

    public final static String CAR_DELETE_RESPONSE_MESSAGE= "Car Successfully Deleted";



}
